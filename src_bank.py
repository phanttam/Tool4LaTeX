...
class Bank(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.group = QGroupBox('Thông tin về Ngân hàng câu hỏi')
        ...
        self.group.button2 = QPushButton('Xem thông tin chung về Ngân hàng đề thi')
        ...
        self.group.grip.label21 = QLabel('[Y]')
        self.group.grip.label22 = QLabel()
        self.group.grip.label23 = QLabel('[B]')
        self.group.grip.label24 = QLabel()
        self.group.grip.label25 = QLabel('[K]')
        self.group.grip.label26 = QLabel()
        self.group.grip.label27 = QLabel('[G]')
        self.group.grip.label28 = QLabel()
        self.group.grip.label29 = QLabel('[T]')
        self.group.grip.label210 = QLabel()
        ...
        self.group.hbox.addWidget(self.group.radio2)
        self.group.hbox.addWidget(self.group.radio1)
        self.group.hbox.addWidget(self.group.button2)
        self.group.hbox2.addWidget(self.group.hbox2.label1)
        self.group.hbox2.addWidget(self.group.hbox2.line1)
        self.group.hbox2.addWidget(self.group.hbox2.button1)
        ...
        self.group2 = QGroupBox('Nạp đề thi vào ngân hàng')
        ...
        self.group2.label1 = QLabel('File(s) ')
        ...
        self.group2.button2 = QPushButton('Nạp đề')
        ...
        self.group3 = QGroupBox('Kiểm tra chuẩn hóa trước khi nạp đề vào Ngân hàng đề thi (Sẽ update chuẩn hóa cấu trúc)')
        ...
        self.group3.label1 = QLabel('File(s)')
        ...
        self.group3.button2 = QPushButton('Kiểm tra')
        ...
        self.group7 = QGroupBox('Update to be continue')
        self.group7.setMinimumHeight(150)
        self.layout().addWidget(self.group3, 1, 1)
        self.layout().addWidget(self.group2, 2, 1)
        self.layout().addWidget(self.group, 3, 1)
        for widget in (self.group, self.group2):
            widget.setMinimumHeight(350)

        self.group2.setMinimumHeight(60)
        self.group3.setMinimumHeight(60)
        self.group5.setMinimumHeight(60)

    def G1_HD(self):
        Form_baithi = 'Nên kiểm file này qua bước chuẩn hóa và kiểm tra ma trận.\r\nBước 1: Nhấn chọn file hoặc nhiều file.\r\nBước 2: Nhấn Nạp đề.'
    ...
    def G1_get_file_2(self):
        ...
        if self.group.radio1.isChecked():
            QMessageBox.question(self, 'Thông báo', 'Ngân hàng Offline chưa xây dựng dữ liệu.\nBạn sử dụng ngân hàng cá nhân nhé!', QMessageBox.Yes)
            self.group.radio2.setChecked(True)
        try:
            Data = []
            ...
            for line in Bank:
                ...
                Data.append((ID[0], 'Dạng ' + ID_information_so(ID[0])[5], '', ID[1], ID[2], ID[3], ID[4], ID[5], ID[6]))
            ...
            Bank_info = Bank_General_Information(Data)
            Mucdo = Bank_info[0]
            TQ = Bank_info[1]
            ...
            QMessageBox.question(self, 'Thông báo', 'Thời gian xử lí: ' + str(int(hour)) + ':' + str(int(minute)) + ':' + str(round(second, 2)) + '\n', QMessageBox.Yes)
        except Exception as err:
            QMessageBox.question(self, 'Thông báo', 'Bạn chưa xây dựng bank', QMessageBox.Yes)
    ...
    def G1_get_file_4(self):
        ...
        if self.group.radio2.isChecked():
            ...
            Sum_T = 0
            Sum_Sum = 0
            List = List_creat(self.group.hbox2.line1.text())
            list_ID = ID_get_list(self.group.line1.text())
            list_ID_n = ID_arrange_bai(list_ID, type_arrange)
            Data = ID_thong_ke_ma_tran_ID6(self.group.line1.text(), list_ID_n)
            Matran_n = []
            for ID in Data:
                ID_mau = ID[0]
                ...
                strY = QTableWidgetItem(str(Sum_Y) + '/' + str(Sum_B) + '/' + str(Sum_K) + '/' + str(Sum_G) + '/' + str(Sum_T))
                Matran_n.append((ID[0], str(Sum_Y), str(Sum_B), str(Sum_K), str(Sum_G), str(Sum_T), ID[3], ID[4], ID[5], ID[6], ID[7]))
                Max_DE = Matrix_Count_Max_Test(Matran_n)
                font = QFont()
                font.setBold(True)
                strY.setFont(font)
                if i < len(Data):
    ...
    def G2_HD(self):
        Form_baithi = 'Bước 1: Nhấn chọn file hoặc nhiều file.\r\nBước 2: Nhấn kiểm tra.\r\nChú ý:\r\n1. Tool chỉ kiểm tra ID nào đã đúng trong list ID6\r\n2. Sau này Tool sẽ kiểm tra về cấu trúc form chuẩn.'
        QMessageBox.question(self, 'Hướng dẫn', Form_baithi, QMessageBox.Yes)
                ...
                if 1 == 1:
                    ...
                    for filename in List:
                        ...
                        if len(ID_loi) != 0:
                            ...
                            Print += '\n'
                            Print += 'Bạn cần gán ID lại file này.\n'
                        else:
                            List_filename.append(filename)

                    if len(List_filename) != 0:
                        Bank_Creat_First(List_filename)
                        Bank_Get_Information()
                    t = time.time() - start_time
                    hour, minute, second = Time_convert(t)
                    if Print == '\nCác file bị lỗi ID là:\n':
                        Print = ''
                    QMessageBox.question(self, 'Thông báo', 'Đã nạp thành công ' + str(len(List_filename)) + ' file vào ngân hàng.\nThời gian xử lí: ' + str(int(hour)) + ':' + str(int(minute)) + ':' + str(round(second, 2)) + Print, QMessageBox.Yes)
    ...
    def G3_get_file_1(self):
        if 1 == 0:
            fname = QFileDialog.getOpenFileName()
            self.group3.line1.setText(fname[0])
        else:
            fname = QFileDialog.getOpenFileNames()
            filename = ''
            for i in fname[0]:
            ...
            for filename in List:
                ...
                if len(ID_loi) != 0:
                    ...
                    for ID in ID_loi:
                        j += 1
                        if ID == '':
                            Print += '\t' + str(j) + '. ID lỗi: Kiểu lỗi sai định dạng ID.\n'
                        else:
                            Print += '\t' + str(j) + '. ID lỗi: ' + ID + '.\n'

                    Print += '\n'
                    Print += 'Bạn cần gán ID lại các file này.'
                    break
                else:
                    Print = 'File(s) của bạn chuẩn về định dạng ID'

            t = time.time() - start_time
            hour, minute, second = Time_convert(t)
            QMessageBox.question(self, 'Thông báo', Print, QMessageBox.Yes)
        except Exception as er:
            QMessageBox.question(self, 'Thông báo', 'Đã xảy ra lỗi.\nLỗi: ' + str(er), QMessageBox.Yes)

    def G5_HD(self):
        Form_baithi = 'Bước 1: Nhấn chọn file.\r\nBước 2: Nhấn Kiểm tra.\r\nChú ý: Version này chỉ kiểm tra lỗi ID, phần sau kiểm tra chuẩn cấu trúc.'
        QMessageBox.question(self, 'Hướng dẫn', Form_baithi, QMessageBox.Yes)
